module types
 ...
 subroutine allocation(lm_1, cw_1, rm_1, dwood_1, sla_1, nind_1,&
     &bminc_1, height_1, cl_inc,cw_inc,ch_inc, cs_inc, cr_inc)   
     use types
     use iso_fortran_env, only : output_unit
 
     !VARIABLES [INPUT] - vem do módulo de estabelecimento/FPC
     real, intent(in) :: lm_1, cw_1, rm_1 ,height_1
     ...
     ! print*, 'cl2 inside alloc', lm_1, cw_1, cr_1, dwood_1,&
      ! &sla_1, nind_1, bminc_1
     !Arrays with values to some variables (generic values)
     dwood = dwood_1*1000000 !(/0.74,0.73,0.59,0.52,0.41,0.44,0.86,0.42,0.64,0.69,0.92,&
     ! &0.60,0.36,0.99,0.59,0.52,0.41,0.44,0.86,0.42/) !atenção para a unidade
     bminc = bminc_1 !(/2.15,2.,2.18,2.6,2.5,1.8,2.3,2.,1.8,2.84,2.25,3.,2.2,1.7,&
     !&1.18,2.6,3.5,2.8,3.3,2./)
     sla = sla_1! (/0.002,0.018,0.009,0.023,0.013,0.039,0.040,0.0028,0.0025,&
     ! &0.027,0.032,0.007,0.013,0.025,0.002,0.008,0.004,0.016,0.023,0.015/)
     nind = nind_1 !(/1.,2.,8.,6.,5.,9.,3.,4.,7.,1.,2.,8.,5.,3.,6.,4.,5.,8.,9.,3./)
     height = height_1 !(/5.,9.,15.,10.9,11.5,18.9,12.6,2.5,14.9,22.5,28.7,23.6,&
     ! &28.8,19.6,13.3,27.6,29.5,21.6,30.,2./)
     lm_ind = lm_1!(/2.15,2.,1.18,1.6,1.5,1.8,0.3,2.,0.8,.84,0.25,1.,0.2,1.7,&
     ! &1.18,1.6,1.5,1.8,0.3,2./)
     cw_ind = cw_1 !(/7.,12.,7.2,8.3,8.8,9.7,7.5,11.5,10.,8.6,7.3,10.3,6.8,9.9,&
     ! &5.3,9.2,15.,12.6,10.7,11.4/)
     rm_ind = rm_1 !(/0.63,0.8,0.9,0.5,1.3,0.9,0.4,1.0,0.56,0.87,0.33,0.97,0.31,&
 ...
 ! ! program allocation !module to test allocation logic module of LPJmL-Fire
 ! subroutine allocation(gc_area,lm_1, cw_1, rm_1, dwood_1, sla_1, nind_1,&
 !     &bminc_1, height_1)    
 !     use types
 !     use iso_fortran_env, only : output_unit
 ...
 !     real, intent(in) :: lm_1, cw_1, rm_1, dwood_1
 !     real, intent(in) :: sla_1, nind_1, bminc_1, height_1
 ...
 !         ! &sla_1, nind_1, bminc_1
 !     !Arrays with values to some variables (generic values)
 !     dwood = dwood_1 !(/0.74,0.73,0.59,0.52,0.41,0.44,0.86,0.42,0.64,0.69,0.92,&
 !     ! &0.60,0.36,0.99,0.59,0.52,0.41,0.44,0.86,0.42/) !atenção para a unidade
 !     bminc = bminc_1 !(/2.15,2.,2.18,2.6,2.5,1.8,2.3,2.,1.8,2.84,2.25,3.,2.2,1.7,&
 !     !&1.18,2.6,3.5,2.8,3.3,2./)
 !     sla = sla_1! (/0.002,0.018,0.009,0.023,0.013,0.039,0.040,0.0028,0.0025,&
