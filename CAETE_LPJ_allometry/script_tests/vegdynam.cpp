...
void mortality_guess(Stand& stand, Patch& patch, const Climate& climate, double fireprob) {
    ...
    //--> Hydraulics-PPX
    // Reset cavitation kills to account for this years cavitational kills only
    patch.cavitation_kills = 0;
    //<-- Hydraulics-PPX
	...
	while (vegetation.isobj) {
		...
		if (!survive(climate,indiv.pft)) {
			...
			if (indiv.pft.lifeform==TREE) {
                ...
                //Hydraulics-PPX -->
                // If hydraulics is set to standard; mort_cav = 0.0
                const double mort_cav = indiv.hydraulic_module->get_hydraulic_failure_mortality();

                if (disable_mort_greff)
                    mort_greff = 0.0;

                mort = 1.0 - (1.0 - mort_min)*(1.0 - mort_greff)*(1.0 - mort_cav);
				...
				// to account for loss of killed individuals
				indiv.reduce_biomass(1.0 - frac_survive, 0.0, false, true, mort_ratio, mort_min, mort_greff, mort_cav);
				...
				if ((patch.stand.is_highlatitude_peatland_stand() || patch.stand.is_true_wetland_stand()) && wetlandlailimit > 0.0) {
					...
					if (!negligible(indiv.cmass_leaf)) {
						...
						if (mort_shade> 0.0) {
							indiv.reduce_biomass(mort_shade, mort_shade, true, false, 0.0);
							allometry(indiv);
						}
...
void fire(Patch& patch,double& fireprob) {
	...
	for (p=0;p<npft;p++) {
		...
		patch.fluxes.report_flux(Fluxes::FIREC_ONLY_LITTER,
		                         mort_fire*(patch.pft[p].litter_leaf + patch.pft[p].litter_sap +
		                                    patch.pft[p].litter_heart + patch.pft[p].litter_repr));
