...
#include "soil.h"
#include "hydraulics/Modules/IHydroDynamics.h"
#include "hydraulics/Modules/HydroDynamicsVPD.h"
#include "hydraulics/Modules/HydroDynamicsMonteith.h"
#include "hydraulics/Math/MCWD_Calculator.h"
...
// --> PPX-Hydraulics
//Todo move them to guessmath.h
/// currently lowest leaf_pot_min of all PFTs (-5.5 MPa) + leaf resistance (0.2 MPa)
const double WILTING_POINT = -3.5; // MPa
/// average emissivity for leaves, Campbell and Norman, (1998)
const double EMISS_LEAF = 0.97;
/// Stefan-Boltzmann constant, W m-2 K-4
const double SIGMA = 5.67e-8;
/// Width of mean leaf [m]
const double LEAF_WIDTH = 0.05;
/// Thermal diffusivity of air [m2 /s]
// https://en.wikipedia.org/wiki/Thermal_diffusivity
const double DIFF_AIR = 1.9e-5;
// Minimum aerodynamic conductance [m/s]
const double MIN_GA = 0.001;
/// Latent heat of vaporizataion of water [kJ kg-1]
const double LAMBDA = 2477.0;
/// Specific heat of air [kJ kg-1 K-1]
const double CP_AIR = 1.005;
/// Density of air [kg m-3]
const double RHO_AIR = 1.1644;
/// Change of latent heat relative to the change in
/// sensible heat of saturated air at 10°C (Koestner et. al 1992) [-]
const double EPS = 1.27;
/// Mega pascal to kilo pascal conversion [kPa/MPa]
const double MPaTokPa = 1000.0;
/// Universal gas constant for an ideal gas [J/mol/K] [kg m^2/s^2 /mol /K]
const double R_IDEAL = 8.3144598;
/// Specifiy gas constant for water vapour (Koestner et. al 1992) [m3 kPa kg-1 K-1]
const double G_V = 0.462;
/// Zero celcius in kelvin [°K]
const double C0 = 273.15;
/// Density of water [kg m-3]
const double RHO_WATER = 998.0;
/// Pychometric constant [kPa K-1]
const double GAMMA = 0.0659;
// <-- PPX-Hydraulics
...
public:
    ...
    /// Cumulative Water Deficit mm (based on const ET of 3.33 mm day-1)
    double cwd_daily_3;
    /// Maximum Cumulative Water Deficit for this year mm (based on const ET of 3.33 mm day-1)
    double mcwd_daily_3;
    /// Cumulative Water Deficit mm (based on const ET of 100 mm month-1)
    double cwd_monthly_100;
    /// Maximum Cumulative Water Deficit for this year mm (based on const ET of 100 mm month-1)
    double mcwd_monthly_100;
...
public:
	...
	Climate(Gridcell& gc):gridcell(gc), mcwd_calculator(*this)  {
...
public:
	...
	enum PerPatchFluxType {
		...
		FIREC,
		/// Carbon flux from atmosphere only from burnt vegetation, not litter (kgC/m2)
		FIREC_NO_LITTER,
		/// Carbon flux from atmosphere only from litter (kgC/m2)
		FIREC_ONLY_LITTER,
	...
	enum PerPFTFluxType {
		...
		ISO,
		/// Monoterpene (mgC/m2)
        /// Evapotranspiration (does not include soil evaporation) (mm) - TP Monthly PFT fluxes 31.07.14
        AET,
        ...
        MORTDISTC,
        /// Carbon turnover from fine root death - TP 13.11.15
        TURNRC,
        /// Carbon turnover from leaf death - TP 13.11.15
        TURNLC,
		MT_APIN,
		MT_BPIN,
		MT_LIMO,
...
public:
    ...
    // --> PPX-Hydraulics
    /// Maximum canopy/big-leaf conductance [mm/s]
    // From https://www.sciencedirect.com/topics/agricultural-and-biological-sciences/canopy-conductance
    double gC_max = 25.0;
    /// Isohydricity wh,ere 1 exterme isohydric and -0.3 extreme anisohydric [Dimless]
    double isohydricity;
    /// Optimal forcing pressure to maintain gc_max [MPa]
    double delta_psi_max;

    /// Slope of cavitation used for xylem, leaf and root cavitation [-]
    // Be carful with the sign of the curve, which flips the behavior:
    // For cavitation the slope should always be negative
    double cav_slope;
    /// Water potential where 50% of xylem conductivity are lost [MPa]
    double psi50_xylem;

    /// sapwood specific conductivity per unit stem area (kg m-1 s-1 MPa-1)
    double ks_max;
    /// specific root conductivity (s-1 MPa-1)
    double kr_max;
    /// specific leaf conductivity (mmol m-2 s-1 MPa-1)
    double kl_max;


    /// Water potential where immediate death due to hydraulic failure occures.
    /// q-% of the xylems are damaged; derived from xylem_cav relation. Typically: q= 0.85 or 0.88
    //double psi_q_death;
    ///// Water potential where no damage occures. Typically q = 0.12;
    //double psi_q_nodamage;

    /// cavitation where damage to xylems is negligible
    double kappa_no_damage = 0.12;

    /// cavation where damage of xylems leads to imediate death
    double kappa_death = 0.88;
    // <-- PPX-Hydraulics
...
struct PlantHydraulics : public Serializable{

public:
    PlantHydraulics();
    void serialize(ArchiveStream &arch) override;

    /// Leaf water potential [MPa]
    double psi_L;
    /// Weighted Soil water potential [MPa]
    double psi_sw;
    /// Previous Weighted Soil water potential [MPa]
    double psi_sw_prev;
    /// Xylem water potential [MPa]
    double psi_xyl;
    /// Forcing pressure [MPa]
    double delta_Psi;
    /// Total plant resistance [m2 MPa s kg-1]
    double R_total;
    /// Root resistance [m2 MPa s kg-1]
    double R_r;
    /// Shoot resistance [m2 MPa s kg-1]
    double R_s;
    /// Leaf resistance [m2 MPa s kg-1]
    double R_L;
    /// Fraction of cavitated roots (-)
    double kappa_r;
    /// Minimum value of cavitated xylems through the course of the year that can be repaired (-)
    double kappa_s_min;
    /// Fraction of cavitated xylem today (-)
    double kappa_s_today;
    /// Fraction of cavitated leaves  (-)
    double kappa_L;
    /// Total stem water flow on fpc basis [kg /m^2 d]
    double J;
    /// Leaf temperature [degree C]
    double temp_leaf;
    /// Aerodynamic conductance running  [m/s]
    double ga_run;
    /// Coupling between canopy and VPD (0-1)  [-]
    /// 0: 100% imposed transpiration, 1: 100% equilibrium transpiration
    double omega;
    /// Equilibrium plant transpiration (not fpc basis)   [mm / s]
    double et_plant_eql;
    /// Imposed plant transpiration (not fpc basis)  [mm / s]
    double et_plant_imp;
    /// Total plant transiration on fpc basis [mm / s]
    double et_plant_total;
    /// Big leaf / canopy conductance for water vapor [mm / s]
    double gC_water;
    /// Big leaf / canopy conductance for photosynthesis  [mm / s]
    double gC;
    /// Is gC regulated by leaf water potential
    bool waterStressedGC;
};
...
public:
	...
	/// GPP this day
	double dgpp;
    /// Stem and root autotrophic respiration this day
    double resp;
    ...
    // --> PPX-Hydraulics
    PlantHydraulics hydraulic_data;
    /// Hydro Dynamics object, it would be better to use a unique pointer
    /// but LPJ-GUESS is a bit old fashioned here...
    std::unique_ptr<IHydroDynamics> hydraulic_module;
    /// Simple pointer to hydro dynamic objects, does not need to be deleted
    /// This is done to avoid (heavy) dynamic pointer casting or the visitor pattern
    /// and used only in the output module
    //HydroDynamicsVPD* hydro_vpd_ptr;
    //HydroDynamicsMonteith* hydro_month_ptr;
    /// Stomatal sensitivity also refered to as greek letter \xi in hydraulics (-)
    double lambda;
    // <-- PPX-Hydraulics
...
public:
	 ...
	 *                        fire only
	 */
	void reduce_biomass(double mortality, double mortality_fire, bool bfire, bool bgreff, double mort_ratio, double mort_bg = 0.0, double mort_greff = 0.0, double mort_cav = 0.0);
...
public:
    ...
    // --> PPX-Hydraulics
    /// Constant A for soil water pot calculation
    double shapeA;
    /// Constant B for soil water pot calculation
    double shapeB;
    // Volumetric water content at permantant wilting point
    double PWPVWC;
    // <-- PPX-Hydraulics
...
public:
    ...
    /// return soil temperature at 25cm depth
    double& get_soil_temp_25_ref() const;
...
public:
	...
	double cmass_lost_bg;
	double cmass_lost_greff;
	double cmass_lost_cav;

	double cmass_lost_heart_bg;
	double cmass_lost_heart_greff;
	double cmass_lost_heart_cav;

	double cmass_lost_sap_bg;
	double cmass_lost_sap_greff;
	double cmass_lost_sap_cav;

	double cmass_lost_leaf_bg;
	double cmass_lost_leaf_greff;
	double cmass_lost_leaf_cav;

	double cmass_lost_root_bg;
	double cmass_lost_root_greff;
	double cmass_lost_root_cav;
...
public:
	...
	/// Total evapotranspiration as sum of soil evaporation and plant (evapo)transpiration
	double et_total;
	...
	/// Number of currently alive and active individuals
	/// Needed for the output module
	size_t n_indivs_alive;
	/// Number of currently alive and active individuals per pft
	/// could also be a member of PatchPft
	std::vector<int> n_indivs_alive_pft;
    /// Killed cohorts by cavitation (this year)
    int cavitation_kills;
...
public:
	...
	/// Id for this gridcell
	size_t get_id() const;

	/// Index of this gridcell
	size_t get_index() const;

	/// number of gridcells for this node/process
	size_t get_n_proc_gridcells() const;

	/// total number of gridcells over all nodes/processes
	size_t get_n_total_gridcells() const;

	/// First simulation year (without spinup)
	size_t get_first_year() const;

	/// Last simulation year (without spinup)
	size_t get_last_year() const;
	...
	/// Set gridcell ID if available
	void set_id(size_t id);

	/// Set number of gridcells
	void set_total_counts(size_t n_proc_cells, size_t n_total_cells);

	/// Set time begin and end
	void set_time_begin_end(int first_year, int last_year);
...
private:
	...
	/// Station ID for this grid cell (if there is any) required for parallel netcdf
	size_t id;

	/// Index of this gridcell in file position
	size_t index;

	/// total number of gridcells (only required for MPI)
	size_t n_total_gridcells;

	/// number of gridcells to simulate for this process
	size_t n_proc_gridcells;

	/// First year of the simulation (read data without spinup)
	size_t first_simulation_year;

	/// Last year of the simulation (read data without spinup)
